...
#include "models.h"
int comparisonsCountRecursive=0;
int comparisonsCountIterative=0;
...
void debugSelectionSort(sortType type, int* data, int size){
    ...
    // running actual sort
    if(type == RECURSIVE){
        comparisonsCountRecursive=0;
        selectionSortRec(data, size, 0);
    }
    else if (type == ITERATIVE){
        comparisonsCountIterative=0;
    ...
    // write a debug message performance related
    printf( type==RECURSIVE?"RECURSIVE - ":"ITERATIVE - " );
    printf("Selection Sort for %d items took: %lf sec",size, elapsedSeconds);
    printf("and %d iterations \n", type==RECURSIVE?comparisonsCountRecursive:comparisonsCountIterative);
}
